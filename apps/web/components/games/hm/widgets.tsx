...
export const Option = ({
  ...
  return (
    <motion.div
      className={cn(
        "bg-white text-black p-4 rounded-2xl flex items-start gap-4 cursor-pointer hover:scale-[1.02] shadow-2xl transition-transform duration-300 ease-linear relative h-40 w-full",
    ...
    >
      ...
      <p className="text-3xl font-semibold m-auto text-center">{text}</p>
...
export const PlayerOption = ({
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className={cn(
        "bg-white group text-black p-4 rounded-2xl flex items-start gap-4 cursor-pointer select-none transition-transform duration-300 ease-linear h-[20dvh]",
        "hover:scale-102 active:scale-105 relative",
        selected && "data-[letter=A]:bg-[var(--umati-yellow)] data-[letter=B]:bg-[var(--umati-sky)] data-[letter=C]:bg-[var(--umati-purple)] data-[letter=D]:bg-[var(--umati-red)] data-[letter=E]:bg-orange-500 data-[letter=F]:bg-[var(--umati-aqua)]",
      ...
      transition={{ duration: 0.15 }}
    >
     {!selected &&  <div data-letter={letter} className="absolute size-8 font-bold rounded-full group-data-[letter=A]:bg-[var(--umati-yellow)] group-data-[letter=B]:bg-[var(--umati-sky)] group-data-[letter=C]:bg-[var(--umati-purple)] group-data-[letter=D]:bg-[var(--umati-red)] group-data-[letter=E]:bg-orange-500 group-data-[letter=F]:bg-[var(--umati-aqua)] flex items-center justify-center text-white">
        {letter}
      </div>}
      <p className="text-xl md:text-2xl font-semibold text-center w-full m-auto">
        {text}
      </p>
    </motion.div>
  );
...
export const PlayerOptions = ({
  ...
  return (
    <div className="grid grid-cols-2 gap-4 max-w-4xl w-full mx-auto px-4">
      {options.map((opt) => (
        <PlayerOption
          key={opt.letter}
          {...opt}
...
export const RoundHost = () => {
  ...
  useEffect(() => {
    ...
    if (state === "ROUND_END") {
      // setDealPhase("returning");
      const t2 = setTimeout(() => setDealPhase("returning"), 4000);
